...
@app.on_message(command(["/ac","/av","ميوزك اليوم"]) & SUDOERS)
async def start(client: Client, message: Message):
    ac_audio = str(len(await get_active_chats()))
    ac_video = str(len(await get_active_video_chats()))
    await message.reply_text(f"<b>✫  احصائيات الميوزك اليوم :</b>\n\n<b>اغنيه : {ac_audio}\n فيديو : {ac_video}</b>", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('✯ اغلاق ✯', callback_data=f"close")]]))
