...
def welcomepic(pic, user, chatname, id, uname):
    background = Image.open("ZeMusic/assets/wel2.png")
    pfp = Image.open(pic).convert("RGBA")
    pfp = circle(pfp)
    pfp = pfp.resize((825, 824))
    draw = ImageDraw.Draw(background)
    font = ImageFont.truetype('ZeMusic/assets/font.ttf', size=110)
    welcome_font = ImageFont.truetype('ZeMusic/assets/font.ttf', size=60)
...
async def auto_state(_, message):
    ...
    if user.status in (
        ...
        if state == "تفعيل":
            ...
            if not A:
                return await message.reply_text("** تم تعطيل الترحيب مسبقا**")
            elif A:
                await wlcm.rm_wlcm(chat_id)
                await message.reply_text(f"**  تم تعطيل الترحيب مسبقا في {message.chat.title}**")
        else:
            await message.reply_text(usage)
    else:
        await message.reply("** امر تفعيل الترحيب خاص بالمسؤولين فقط**")
